...
function footer(lang, isHome=false) {
  return `<footer class="site-footer"><div class="wrap footer-grid"><div><a class="footer-brand" href="${homeHref(lang)}">Jitra2Stay</a><p>${t(lang,'Rumah untuk keluarga.<br>Di Jitra, Kedah.','A home for your family.<br>In Jitra, Kedah.')}</p></div><div><h2>${t(lang,'Hubungi owner','Contact the owner')}</h2><a href="tel:+${config.business.phone}">${e(config.business.phoneDisplay)}</a><a href="tel:+${config.business.secondaryPhone}">${e(config.business.secondaryPhoneDisplay)} ${t(lang,'(panggilan)','(calls)')}</a><a href="mailto:${e(config.business.email)}">${e(config.business.email)}</a></div><div><h2>${t(lang,'Maklumat penginapan','Stay information')}</h2><a href="${pageHref('policies',lang)}">${t(lang,'Polisi & house rules','Policies & house rules')}</a><a href="${config.business.mapUrl}" target="_blank" rel="noopener">Google Maps ${icon('arrow')}</a><a href="${homeHref(lang)}#semak-tarikh">${t(lang,'Tanya tarikh','Ask about dates')}</a></div></div><div class="wrap footer-bottom"><span>© ${new Date().getUTCFullYear()} Jitra2Stay</span><span>${t(lang,'Tarikh dan tempahan disahkan oleh owner.','Dates and bookings are confirmed by the owner.')}</span></div></footer>
