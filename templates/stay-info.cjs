...
function renderNearby(lang) {
  return `<div class="nearby-places"><h3>${t(lang,'Tempat berdekatan','Nearby places')}</h3><p class="nearby-note">${t(lang,'Anggaran perjalanan dari rumah; masa sebenar bergantung pada trafik dan destinasi.','Approximate travel times from the house; actual times depend on traffic and your destination.')}</p><div class="nearby-grid">${config.nearby.map(place=>`<details><summary><span class="nearby-time">${e(place[lang][0])}</span><span>${e(place[lang][1])}</span>${icon('arrow')}</summary><p>${e(place[lang][2])}</p></details>`).join('')}</div></div>`;
}
