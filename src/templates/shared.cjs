...
function icon(name) {
  const shapes = {
    ...
    washer: '<rect x="4" y="2" width="16" height="20" rx="2"/><path d="M4 7h16M7 4.5h.01M10 4.5h.01"/><circle cx="12" cy="14" r="5"/><path d="M7.5 14c2-2 3 2 5 0s3-1 4 0"/>',
    iron: '<path d="M3 17h18l-3-9H9l-6 9Zm0 0v3h18v-3M9 8V5h6a3 3 0 0 1 3 3M8 16h.01M12 16h.01M16 16h.01"/>',
...
function footer(lang, isHome=false) {
  return `<footer class="site-footer">
    <div class="wrap footer-grid">
      ...
      <nav class="footer-stay" aria-labelledby="footerStayTitle"><h2 id="footerStayTitle">${t(lang,'Penginapan','Your stay')}</h2><ul class="footer-links"><li><a href="${pageHref('policies',lang)}">${t(lang,'Polisi & house rules','Policies & house rules')}</a></li><li><a href="${homeHref(lang)}#kalendar">${t(lang,'Kalendar & tetamu','Calendar & guests')}</a></li><li><a href="${pageHref('guest-admin',lang)}">${t(lang,'Urus tetamu','Manage guests')}</a></li><li><a href="${homeHref(lang)}#faq">${t(lang,'Soalan lazim','Common questions')}</a></li><li><a href="${config.business.mapUrl}" target="_blank" rel="noopener">Google Maps ${icon('arrow')}</a></li><li><a href="${homeHref(lang)}#semak-tarikh">${t(lang,'Tanya tarikh','Ask about dates')}</a></li></ul></nav>
    </div>
...
function compactHeader(lang, pairedPath) {
  return `<header class="site-header guest-site-header"><div class="wrap guest-header-inner"><a class="brand" href="${homeHref(lang)}">${icon('home')}<span>Jitra2Stay<span class="brand-place">${t(lang,'PENGURUSAN TETAMU','GUEST MANAGEMENT')}</span></span></a><div class="guest-header-tools"><a href="${e(pairedPath)}" lang="${lang==='en'?'ms':'en'}" hreflang="${lang==='en'?'ms':'en'}">${lang==='en'?'BM':'EN'}</a><button id="themeToggle" class="icon-button" type="button" hidden aria-label="${t(lang,'Tukar tema','Change theme')}" aria-pressed="false">${icon('sun')}</button></div></div></header>`;
}
function compactFooter(lang) {
  return `<footer class="guest-site-footer wrap"><a class="text-link" href="${homeHref(lang)}#kalendar">${t(lang,'Kembali ke laman utama','Back to home')} ${icon('arrow')}</a><span>© ${new Date().getUTCFullYear()} Jitra2Stay</span></footer>`;
}
function layout({lang='ms', title, description, path='', pairedPath='en.html', body, isHome=false, noindex=false, pageAssets='', compact=false, mobilePage=''}) {
  const origin=config.business.siteUrl.replace(/\/$/,'');
  const canonical=`${origin}/${path}`;
  const msPath=lang==='ms'?path:pairedPath, enPath=lang==='en'?path:pairedPath;
  const b=config.business;
  const guestAdmin=pageAssets==='guest-admin';
  const browse=isHome || Boolean(mobilePage);
  const view=isHome?'home':mobilePage||'document';
  ...
  return `<!doctype html>
<html lang="${lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>${e(title)}</title><meta name="description" content="${e(description)}"><meta name="robots" content="${noindex?'noindex,follow':'index,follow'}"><meta name="referrer" content="strict-origin-when-cross-origin"><meta name="color-scheme" content="light dark"><meta name="theme-color" content="#174b3a"><meta name="format-detection" content="telephone=no"><link rel="canonical" href="${e(canonical)}">
