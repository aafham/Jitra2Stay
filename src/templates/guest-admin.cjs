...
function renderGuestAdmin(lang = 'ms') {
  ...
  const body = `<div class="guest-admin wrap" data-guest-admin>
    ...
    <section class="guest-panel guest-pin-panel" id="guestPinPanel" aria-labelledby="guestPinTitle">
      ...
      <p class="guest-private-note">${text('PIN untuk urus rekod. Nama dan tarikh dipaparkan kepada umum, termasuk bilangan orang jika diisi. Tujuan hanya untuk pengurusan.', 'Use your PIN to manage records. Names, dates and any guest count provided are public. The purpose is only shown to management.')}</p>
    </section>
    <div id="guestWorkspace" hidden>
      ...
      <div class="guest-workspace-grid">
        <section class="guest-panel guest-form-panel" aria-labelledby="guestFormTitle">
          ...
          <p class="guest-form-intro">${text('Cukup isi tarikh masuk, tarikh keluar dan nama. Selepas itu, tekan Simpan.', 'Just enter check-in, check-out and the guest name. Then choose Save.')}</p>
          <form id="guestDetailsForm" novalidate autocomplete="off">
            <p class="guest-required-note">${text('3 maklumat wajib', '3 required details')}</p>
            <div class="guest-date-fields">
              ${field('guestCheckIn', `<span class="guest-field-step" aria-hidden="true">1</span>${text('Tarikh masuk', 'Check-in date')}`, '<input id="guestCheckIn" name="check_in" type="date" required aria-describedby="guestCheckInHint guestCheckInError">', text('Masuk mulai 3 petang.', 'Check in from 3 pm.'))}
              ${field('guestCheckOut', `<span class="guest-field-step" aria-hidden="true">2</span>${text('Tarikh keluar', 'Check-out date')}`, '<input id="guestCheckOut" name="check_out" type="date" required aria-describedby="guestCheckOutHint guestCheckOutError">', text('Keluar sebelum 12 tengah hari.', 'Check out by 12 noon.'))}
            </div>
            ...
            ${field('guestName', `<span class="guest-field-step" aria-hidden="true">3</span>${text('Nama tetamu', 'Guest name')}`, `<input id="guestName" name="guest_name" type="text" required maxlength="120" autocomplete="off" aria-describedby="guestNameHint guestNameError" placeholder="${text('Nama wakil tempahan', 'Booking contact name')}">`, text('Nama ini dipaparkan dalam senarai tetamu di laman utama.', 'This name appears in the guest list on the home page.'))}
            <details class="guest-optional-details" id="guestOptionalDetails"><summary><span>${text('Maklumat tambahan', 'Additional details')}<small>${text('Bilangan orang & tujuan · tak wajib', 'Guest count & purpose · optional')}</small></span></summary><div class="guest-optional-content">
            ${field('guestCount', `${text('Bilangan orang', 'Number of guests')} <span class="guest-optional">(${text('tak wajib', 'optional')})</span>`, `<input id="guestCount" name="guest_count" type="number" inputmode="numeric" min="1" max="20" step="1" aria-describedby="guestCountHint guestCountError" placeholder="${text('Boleh kosongkan', 'Can be left blank')}">`, text('Jika sudah tahu, isi jumlah semua tetamu termasuk kanak-kanak. Maksimum 20 orang.', 'If known, include everyone staying, including children. Maximum 20 guests.'))}
            ${field('guestPurpose', `${text('Tujuan', 'Purpose')} <span class="guest-optional">(${text('tak wajib', 'optional')})</span>`, `<textarea id="guestPurpose" name="purpose" rows="3" maxlength="500" aria-describedby="guestPurposeHint guestPurposeError" placeholder="${text('Contoh: kenduri, konvokesyen atau bercuti', 'For example: a wedding, graduation or holiday')}"></textarea>`, text('Catatan pengurusan sahaja; tidak dipaparkan kepada umum.', 'A management note only; not shown publicly.'))}
            </div></details>
            ...
            <p class="guest-save-note">${text('Rekod disimpan, kemudian kalendar dibuka semula.', 'Your record is saved, then the calendar opens again.')}</p>
          </form>
        </section>
